...
部分服务商（如京东云）不支持 `content` 数组格式，LLM-ROUTE 会自动将数组格式转换为字符串格式。
...
此功能对 `/v1/messages` 请求自动启用，无需额外配置。只需确保上游配置的 `protocol` 为 `anthropic`：

```yaml
upstreams:
  anthropic:
    url: https://modelservice.jdcloud.com/coding/anthropic
    protocol: anthropic  # 自动启用格式转换
```

**适用场景：**
- 京东云模型服务
- 其他不支持 `content` 数组格式的服务商
...
**v1.3.1 (2026-04-24)** 新增：
- ✅ Anthropic Messages API 格式转换（`content` 数组 → 字符串）
- ✅ 提高京东云等服务商的兼容性
