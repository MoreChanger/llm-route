...
data: {"type":"response.function_call_arguments.done","arguments":"{\"city\":\"Beijing\"}"}
...
| `previous_response_id` | 从会话存储获取历史 |
...
会话历史存储在内存中，重启服务后清空。通过 `previous_response_id` 关联：
- 每个响应都有唯一 ID
- 客户端可通过 `previous_response_id` 获取历史上下文
